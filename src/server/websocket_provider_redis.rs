...
use crate::app::service_redis::{RedisApplicationService,RedisFactory, DataStructure};
    ...
    T: Debug + Clone + Serialize + DeserializeOwned + 'static + std::marker::Send
{
    ...
    async fn load(&self)-> Result<(), WebsocketServerError> {
        ...
        let result: ApplicationModel<Websocket> = persistence.find_by_id("websocket:mapping")
            .await
